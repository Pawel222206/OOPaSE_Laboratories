...
namespace GAMEGANGINE
{
    void PlayerCharacter::updatehealth(int sufferedDamage)
    {
        ...
        notifyObservers(sufferedDamage);
    }
    ...
    void PlayerCharacter::removeObserver(Observer* observer)
    {
        observers.erase(remove(observers.begin(), observers.end(), observer), observers.end());
    }
    void PlayerCharacter::notifyObservers(int sufferedDamage)
    {
        for (Observer* observer : observers)
        {
            observer->update(sufferedDamage);
        }
    }
}
